...
df = df.rename(columns={'0': 'X1', '1': 'X2', '2': 'X3', '3': 'X4', '4': 'Y'})
# так как ответы у нас строки - нужно перейти к численным значениям
df['Y'] = df['Y'].replace(['Iris-setosa', 'Iris-versicolor', 'Iris-virginica'], [1, 0, -1])

# возьмем 4-й столбец 
y_copy = df.iloc[:, 4].values

#a = [[0] * 150 for _ in range(3)]
...
XandY = np.concatenate ((df, y), axis= 1)
...
outputSize = 1 if len(y.shape)<2 else y.shape[1] # количество выходных сигналов равно количеству классов задачи
...
def backward(learning_rate, target, net_output, layers):
    ...
    err = (target - net_output)                               #net_output.T
    #err = err.T
    # print("target = ", target)
...
for i in range(iterations):
    ...
    # x_rand = X.iloc[np.random.permutation(len(XandY))]
        
    new_ind = np.random.permutation(N)  
    x_rand = XandY[new_ind]
   ...
   # for j in range(N):
    train(x_i[0, 0:5], x_i[0, 5:], learning_rate)

    
    print("На итерации: " + str(i) + ' || ' + "Средняя ошибка: " + str(np.mean(np.square(y - predict(x_i[0, 0:5])))))
...
# pr = predict(X)
# print("ошибка на всей выборке", sum(abs(y-(pr>0.5))))
